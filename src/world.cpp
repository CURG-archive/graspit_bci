...
//#define PROF_ENABLED
#include "profiling.h"
PROF_DECLARE(WORLD_FIND_CONTACTS);PROF_DECLARE(WORLD_COLLISION_REPORT);PROF_DECLARE(WORLD_NO_COLLISION);PROF_DECLARE(WORLD_GET_DIST);PROF_DECLARE(WORLD_POINT_TO_BODY_DISTANCE);PROF_DECLARE(WORLD_FIND_VIRTUAL_CONTACTS);PROF_DECLARE(WORLD_FIND_REGION);PROF_DECLARE(DYNAMICS);
...
*/
World::World(QObject *parent, const char *name, IVmgr *mgr) : QObject(parent, name), currentPlanner(NULL) {
    myIVmgr = mgr;
