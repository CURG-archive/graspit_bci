...
}PROF_DECLARE(GET_GRASPS);PROF_DECLARE(GET_GRASPS_CALL);

void DBaseDlg::loadGraspButton_clicked() {
    PROF_RESET_ALL;PROF_START_TIMER(GET_GRASPS);
    ...
    graspIndexLabel->setText(numCurrent + "/" + numTotal);PROF_STOP_TIMER(GET_GRASPS);PROF_PRINT_ALL;
}
