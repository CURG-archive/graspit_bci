...
public:
    ...
    void emitGoToNextState1() {
        emit goToNextState1();
    }

    void emitGoToNextState2() {
        emit goToNextState2();
    }

    void emitGoToPreviousState() {
    ...
    void emitTargetBodyChanged(GraspableBody *b) {
        emit targetBodyChanged(b);
    }

    void emitResetStateMachine() {
        resetStateMachine();
    }

    void emitCursorPosition(double x, double y) {
        emit cursorPosition(x, y);
    }

    void emitDrawableFrame(DrawableFrame *drawableFrame) {
        emit drawShapes(drawableFrame);
    }
    ...
    void emitAnalyzeGrasp(const GraspPlanningState *gps) {
        emit analyzeGrasp(gps);
    }

    void emitAnalyzeNextGrasp() {
        emit analyzeNextGrasp();
    }

    void emitRunObjectRecognition() {
    }

    void emitAnalyzeApproachDir(GraspPlanningState *gs) {
        emit analyzeApproachDir(gs);
    }
