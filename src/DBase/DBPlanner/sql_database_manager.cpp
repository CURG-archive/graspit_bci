...
//#define PROF_ENABLED
#include "profiling.h"
PROF_DECLARE(GET_GRASPS_SQL);PROF_DECLARE(GET_GRASPS_GETCOLUMN);PROF_DECLARE(GET_GRASPS_GETFIELD);
...
//! Load a Model from the database and populate its fields.
    ...
    bool SqlDatabaseManager::GetNeighbors(const Model &model,
        ...
        for (int row = 0; row < num_rows; ++row) {
            ...
            if (!load_model_functor(neighbor_model, row) ||
                    !results.GetField(distance_column, row, &distance))
                return false;
    ...
    bool SqlDatabaseManager::ModelList(vector<Model *> *model_list,
            FilterList::FilterType filter, const QString &filename) const {
        if (model_list == NULL) return false;
        Table results;
        std::string query_string = "SELECT * FROM get_models() " +
                FilterList::GetWhereClause(filter, filename) + ";";
        std::cout << "Model Query String:" << query_string << std::endl;
        if (!database_.Query(query_string,
                &results))
            return false;
        // Turn each database row into a Model.
        LoadModelFunctor load_model_functor(results, model_root_);
        ...
        for (int row = 0; row < num_rows; ++row) {
            ...
            if (!load_model_functor(model, row)) {
                std::cout << "Load model functor failed:" << model->ModelName() << std::endl;
                return false;
            }
...
/*! This is a helper to filter out duplicate code from the GetXXXList functions.
    ...
    bool SqlDatabaseManager::GetGrasps(const Model &model,
        ...
        Table results;PROF_START_TIMER(GET_GRASPS_SQL);
        std::string query_string = "SELECT * FROM get_grasps('" + model.ModelName() +
                "','" + hand_name + "');";
        std::cout << "Query string: " << query_string << std::endl;
        if (!database_.Query(query_string, &results))
            return false;PROF_STOP_TIMER(GET_GRASPS_SQL);
        ...
        int pregrasp_joints_column, grasp_joints_column,
                ...
                grasp_contacts_column, grasp_source_name_column;PROF_START_TIMER(GET_GRASPS_GETCOLUMN);
        if (!results.GetColumnIndex("grasp_pregrasp_joints", &pregrasp_joints_column) ||
            ...
            return false;PROF_STOP_TIMER(GET_GRASPS_GETCOLUMN);
        ...
        for (int row = 0; row < num_rows; ++row) {
            ...
            if (!results.GetField(pregrasp_joints_column, row, &pregrasp_joints) ||
                ...
                return false;PROF_STOP_TIMER(GET_GRASPS_GETFIELD);
