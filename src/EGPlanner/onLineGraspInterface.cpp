...
/*!	Given a list of f-c grasps, this function chooses one and shapes 
	...
	CHANGED! now this only finds the state that the hand can acheive
*/
GraspPlanningState *
OnLineGraspInterface::updateHand(const std::list<GraspPlanningState *> *solutionList) {
    ...
    if (s) {
        ...
        //mHand->forceDOFVals(finalDof);//CHANGED!
        mHand->forceDOFVals(savedDof);//CHANGED! now it doesn't move the reference hand while planning
#ifdef HARDWARE_LIB
		if (mBarrettHand) {
