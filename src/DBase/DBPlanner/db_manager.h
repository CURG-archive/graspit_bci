...
//! to a SQL "WHERE" clause that can be tacked onto a query.
    ...
    public:
        ...
        enum FilterType {
            NONE, HAS_HUMAN_GRASPS, HAS_HUMAN_REFINED_GRASPS, USE_FILE_NAME
        };

        //! Maps a FilterType enum to a SQL "WHERE" clause.
        static string GetWhereClause(FilterType filter, QString objectFileName = "") {
            ...
            } else if (filter == USE_FILE_NAME) {
                return "WHERE strpos(original_model_geometry_path,'" +
                        objectFileName.toStdString() +
                        "') > 0";
            } else {
                return "";
            }
...
//! Pure virtual base class for interfacing GraspIt with an unspecified Grasp Database.
    ...
    public:
        ...
        virtual bool ModelList(vector<Model *> *model_list,
                FilterList::FilterType filter = FilterList::NONE, const QString &filename = "") const = 0;
