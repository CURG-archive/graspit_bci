    ...
    void recursion(const Node *n1, const Node *n2, RecursionCallback *rc) {
        ...
        if (n1->isLeaf()) {
            if (!n2 || n2->isLeaf()) {
                ...
                rc->leafTest(static_cast<const Leaf *>(n1), static_cast<const Leaf *>(n2));PROF_STOP_TIMER(COLLISION_RECURSION);
                return;
            } else {
        ...
        if (rc->distanceTest(d1)) {
            DBGST(c1->mark(true));
            DBGST(if (s) s->mark(true));PROF_STOP_TIMER(COLLISION_RECURSION);
            //make sure we don't change the order of the models
            if (!nodeSwap) {
                recursion(c1, s, rc);
            } else {
