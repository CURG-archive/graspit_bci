...
SearchEnergy::compliantEnergy() const {
    PROF_RESET(QS);PROF_TIMER_FUNC(QS);
...
SearchEnergy::dynamicAutograspEnergy() const {
    ...
    QObject::connect(mHand->getWorld(), SIGNAL(dynamicsError(
            const char*)),
            this, SLOT(dynamicsError(
            const char*)));
    //loop until dynamics is done
    mDynamicsError = false;
    int steps = 0;
    int stepFailsafe = 1500;
    int autoGraspDone = 0;
    int afterSteps = 100;
    ...
    QObject::disconnect(mHand->getWorld(), SIGNAL(dynamicsError(
            const char*)),
            this, SLOT(dynamicsError(
            const char*)));
...
ClosureSearchEnergy::analyzeState(bool &isLegal, double &stateEnergy, const GraspPlanningState *state, bool noChange) {
    ...
    if (mAvoidList) {
        std::list<GraspPlanningState *>::const_iterator it;
        int i = 0;
